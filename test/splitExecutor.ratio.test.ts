...
describe("assertSplitRatiosSumToOne", () => {
  ...
  it("rejects out-of-range ratios even when their hostile aggregate equals 1", () => {
    expect(() => assertSplitRatiosSumToOne([r(A, -1), r(B, 2)])).toThrow(
      SplitRatioSumError
    );
  });
...
describe("splitExecutor — ratio pre-flight", () => {
  ...
  it("rejects individually invalid ratios before any other pre-flight work", async () => {
    await expect(
      splitExecutor([r(A, -1), r(B, 2)], { skipCapacityCheck: true })
    ).rejects.toBeInstanceOf(SplitRatioSumError);
  });
